...
public class NotificationReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (note != null && note.getRepeatDays() != 0) {
    ...
    private static void createChannel(Context ctx) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            NotificationChannel ch = new NotificationChannel(CHANNEL_ID, "Заметки", NotificationManager.IMPORTANCE_HIGH);
            ch.setDescription("Напоминания заметок");
